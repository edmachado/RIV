...
public class WindowsService {
	...
	public static void removeService(String service) {
		...
		if (isServiceActive(service)) System.out.println("Couldn't remove service.");
	}
	...
	public static boolean isServiceExist(String serviceName)  {
		try {
			Process process = Runtime.getRuntime().exec(String.format("sc query %s", serviceName));
        	...
        	return out.contains("SERVICE_NAME:") && out.contains(serviceName) && out.contains("TYPE") && !out.contains("FAILED");
        } catch (Exception e) {
        	e.printStackTrace(System.out);
        	return false;
        }
