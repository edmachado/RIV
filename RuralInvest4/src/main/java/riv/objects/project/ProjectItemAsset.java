...
public class ProjectItemAsset extends ProjectItem implements ProjectInvestment {
	static final Logger LOG = LoggerFactory.getLogger(ProjectItemAsset.class);
	...
	public String testingProperties(RivConfig rivConfig) {
		   ...
		   for (int i=0;i<getProject().getDonors().size();i++) {
			   sb.append("step7.asset."+(this.getOrderBy()+1)+".donations."+(i+1)+"="+cf.formatCurrency(donations.containsKey(i) ? donations.get(i) : 0.0, CurrencyFormat.ALL)+lineSeparator);
		   }
