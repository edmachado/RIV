...
public class ProjectItemService extends ProjectItem implements ProjectInvestment {
	static final Logger LOG = LoggerFactory.getLogger(ProjectItemService.class);
	...
	public Double getFinanced() {
	 ...
	 public String testingProperties(RivConfig rivConfig) {
		   ...
		   for (int i=0;i<getProject().getDonors().size();i++) {
			   sb.append("step7.service."+(this.getOrderBy()+1)+".donations."+(i+1)+"="+cf.formatCurrency(donations.containsKey(i) ? donations.get(i) : 0.0, CurrencyFormat.ALL)+lineSeparator);
		   }
