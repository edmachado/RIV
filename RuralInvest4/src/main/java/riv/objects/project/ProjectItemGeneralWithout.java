...
public class ProjectItemGeneralWithout extends ProjectItem {//implements GeneralCosts {
	...
	public Double getExternal() {
	 ...
	 public String testingProperties(RivConfig rivConfig) {
		   ...
		   sb.append("step8.supplyWo."+(this.getOrderBy()+1)+".unitNum="+rivConfig.getSetting().getDecimalFormat().format(unitNum)+System.lineSeparator());
		   ...
		   sb.append("step8.supplyWo."+(this.getOrderBy()+1)+".ownResources="+cf.formatCurrency(OwnResources, CurrencyFormat.ALL)+System.lineSeparator());
