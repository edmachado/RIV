...
public class ProjectItemNongenMaintenance extends ProjectItemNongenBase {
	...
	public String testingProperties(RivConfig rivConfig) {
		String lineSeparator = System.getProperty("line.separator");
		   ...
		   sb.append("step8.general"+(this.getOrderBy()+1)+".donated="+cf.formatCurrency(getDonated(), CurrencyFormat.ALL)+lineSeparator);
		   for (int i=0;i<getProject().getDonors().size();i++) {
			   sb.append("step8.general"+(this.getOrderBy()+1)+".donations."+(i+1)+"="+cf.formatCurrency(this.getDonations().containsKey(i) ? this.getDonations().get(i) : 0.0, CurrencyFormat.ALL)+lineSeparator);
		   }
