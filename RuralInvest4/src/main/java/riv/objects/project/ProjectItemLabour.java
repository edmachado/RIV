...
public class ProjectItemLabour extends ProjectItem implements ProjectInvestment {
	static final Logger LOG = LoggerFactory.getLogger(ProjectItemLabour.class);
	...
	public Double getFinanced() {
	   ...
	   public String testingProperties(RivConfig rivConfig) {
		   ...
		   for (int i=0;i<getProject().getDonors().size();i++) {
			   sb.append("step7.labour."+(this.getOrderBy()+1)+".donations."+(i+1)+"="+cf.formatCurrency(donations.containsKey(i) ? donations.get(i) : 0.0, CurrencyFormat.ALL)+lineSeparator);
		   }
