		...
		<c:if test="${year eq 0 or perYear}">
			...
			<div id="${type}${year}<c:if test="${without}">Without</c:if>" style="${beginStyle}">
				...
				<display:table list="${costs.get(year)}" id="row" cellspacing="0" cellpadding="0" export="false" htmlId="${htmlId}Table"> 
					...
					<display:column titleKey="${type}.description" property="description" sortable="true" style="text-align:${left};" headerClass="left"/>
					<display:column titleKey="${type}.unitType" sortable="true" style="text-align:${left};" headerClass="left">
						...
						<c:if test="${fn:contains(type,'Personnel') }">
							<c:if test="${row.unitType=='0'}"><spring:message code="units.pyears"/></c:if>
							<c:if test="${row.unitType=='1'}"><spring:message code="units.pmonths"/></c:if>
							<c:if test="${row.unitType=='2'}"><spring:message code="units.pweeks"/></c:if>
					...
					<display:column title="&nbsp;">
						<c:if test="${row.linked}"><img src="../../img/linked.png" width="16" height="16" border="0"></c:if>
						<c:if test="${not row.linked}"><img src="../../img/spacer.gif" width="16" height="16" border="0"></c:if>
					</display:column>
					<c:if test="${accessOK}">
