...
<c:set var="qtyText">
	<c:if test="${project.incomeGen}"><spring:message code="projectBlock.pattern${noCycles}.qty"/></c:if>
	<c:if test="${not project.incomeGen}"><spring:message code="projectActivity.pattern${noCycles}.qty"/></c:if>
</c:set>
...
<tbody>
		...
		<tr class="odd">
			<td id="prodDescription" style="text-align:left; font-size : 11px;">${qtyText}</td>
